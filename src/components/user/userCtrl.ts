...
const userCtrl = {
  ...
  saveLastSearch: async (userRequest: Request, res: Response) => {
    try {
      ...
      if (user.last_search.length <= 1) return res.status(200).json(true);
      const exist = user.last_search.find((last_s) => last_s.toString() === track._id.toString());
      if (exist) {
        const index = user.last_search.findIndex((last_s) => last_s.toString() === track._id.toString());
